...
#### 📱 スマホ表示（モバイルレイアウト）最適化 ＆ Google AdSense 広告スロット実装

**背景・目的**:
スマホ実機での表示時に、グローバルナビゲーションがぎゅうぎゅうに折れ曲がり（「ホ\nー\nム」等）見栄えが著しく崩れていた問題、およびヒーローバナーの画面占有率・カテゴリバーの改行問題を解消。将来的な Google AdSense 導入（審査・収益化）を見据えた広告配置枠を整備。

**変更・追加したファイル:**

| ファイル | 内容 |
|---|---|
| `src/build_portal.py` | モバイル用CSSメディアクエリ追加（ピル型チップ式横スクロールナビ、コンパクトヒーロー、1カラム記事カード、テーブル横スクロール）。AdSenseインフィード広告スロット（2記事目後）およびサイドバー広告スロット（300x250レクタングル）を追加 |
| `src/template.html` | 個別記事用テンプレートのレスポンシブ最適化、AdSense記事内広告スロット（記事上部レスポンシブ広告・まとめ直下の記事下部広告）を追加 |
| `scratch/update_article_htmls.py` | 既存の全5記事HTML（`output/2026-*.html`）を新テンプレートで一括再生成 |
| `.gitignore` | 不要なキャッシュ・コンパイルファイル（`__pycache__/`, `*.pyc`）の確実な除外設定 |
| `SPEC.md` | バージョン 1.4.0 へ更新。モバイル最適化およびAdSense広告配置枠の仕様を反映 |
| `WORKLOG.md` | 本作業ログセッションを追記 |

**検証結果:**
- **ブラウザ検証（iPhoneサイズ: 390px × 844px）**:
  - ナビゲーションバーが指でスワイプできるピル型チップになり、文字の途中で絶対に不自然な改行が発生しないことを確認。
  - ヒーローバナーが程よい高さに収まり、ファーストビューに1記事目のサムネイル・タイトルが自然に入り込む快適なレイアウトを実現。
  - カテゴリフィルターバーも横スクロール可能で、タップによる絞り込みが正常に連動。
  - 記事一覧の2記事目後にAdSenseインフィード広告スロットが違和感なく配置されていることを確認。
  - サイドバーウィジェットおよび個別記事内（見出し手前、記事末尾）にAdSense広告スロットが綺麗に配置されることを確認。
