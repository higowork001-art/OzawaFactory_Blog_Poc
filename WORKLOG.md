...
#### 🛠️ スマホレイアウト全面改修＆ユーザーフィードバック対応（5項目）

**背景・目的**:
スマホ表示時の横スクロールの完全防止、個別記事ページでの文字切れ防止、重複ウィジェットの整理、記事一覧のページネーション（MAX 5件）対応、および「オリジナル商品」へのワンタップ遷移を実現。

**変更点・実装内容:**

1. **横スクロールの完全禁止**:
   - `html, body { overflow-x: hidden; width: 100%; max-width: 100vw; }` を適用し、はみ出しによる画面のガタつき・横スクロールを完全に抑止。
   - グローバルナビとカテゴリタブを `flex-wrap: wrap` でスマホ画面幅に応じて自然に折り返すピル型チップボタンに改修。

2. **個別記事ページ（`template.html`）の左側文字切れ解消**:
   - `.container` の幅を `calc(100% - 32px)`、左右パディングを `20px` に設定し、`word-break: break-word` を追加。スマホ画面の端で文字が隠れる問題を完全に解消。

3. **サイドバーの「カテゴリで絞り込み」重複枠を削除**:
   - 記事一覧上段にすでにカテゴリフィルタータブが設置されているため、サイドバー内の「📂 カテゴリで絞り込み」ウィジェットを削除してスッキリ整理。

4. **記事一覧のページネーション（MAX 5件）機能**:
   - `src/build_portal.py` 内に JavaScript ページネーションロジック（`ITEMS_PER_PAGE = 5`）を組み込み。
   - 5件を超える場合は「← 前のページ / 次のページ →」ナビゲーションが自動生成され、カテゴリ絞り込み時とも完全連動。

5. **最上段ナビゲーションに「🛍️ オリジナル商品」ボタン追加**:
   - ヘッダーナビ（「ホーム」「このサイトについて」等と並ぶ位置）に目立つアクセントカラーで「🛍️ オリジナル商品」ボタンを追加。
   - タップするとページ最下部のオリジナル商品公式ストア案内ウィジェット（`#shop-section`）へスムーズにスクロール。

**検証結果:**
- ブラウザサブエージェントにより、390px × 844px ビューポートで以下を確認：
  - トップページで横スクロールが一切発生せず、綺麗に全要素が収まっていること。
  - 「🛍️ オリジナル商品」ボタンをタップすると最下段のストアウィジェットへスムーズにスクロールすること。
